...
echo ">>> Test Scaling to Existing Hosts (Log and TX nodes on existing targets)"
...
# Target nodes are existing hosts
TARGET_IP_1="192.168.122.24"  # First target (same as TX_IP_1)
NEW_TX_PORT_1=6379

TARGET_IP_2="192.168.122.25"  # Second target (same as TX_IP_2)
NEW_TX_PORT_2=6389
...
run_command "$CLI_LEADER_1 set existing_host_test init_value" "OK"
run_command "$CLI_LEADER_1 get existing_host_test" "init_value"

# 2. Add new log nodes to both existing hosts
NEW_LOG_PORT_1=9700
NEW_LOG_PORT_2=9800
echo ">>> STEP 2: Adding new log nodes to existing hosts" 
...
# 3. Now add TX nodes to both existing hosts
echo ">>> STEP 3: Adding new TX nodes to existing hosts" 
...
# 5. Test data replication across all nodes after restarting the cluster
...
echo ">>> STEP 6: Removing TX nodes from existing hosts" 
...
echo ">>> STEP 7: Removing log nodes from existing hosts" 
...
run_command "$CLI_NEW_TX_1 ping" "PONG"
run_command "$CLI_LEADER_2 get existing_host_test" "init_value"
...
echo "Scaling to Existing Hosts (Log+TX) tests PASSED !!!" 